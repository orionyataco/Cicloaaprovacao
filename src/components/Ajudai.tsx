...
export function Ajudai() {
  ...
  // Indica se o usuário já enviou perguntas
  const hasUserAsked = messages.some(m => m.sender === 'user');
  // As sugestões começam abertas apenas se o usuário ainda não tiver feito perguntas
  const [showSuggestions, setShowSuggestions] = useState<boolean>(() => !hasUserAsked);
  ...
  const handleSend = async (textToSend?: string) => {
    ...
    // As sugestões são ocultadas assim que o usuário faz uma pergunta
...
5. Seja motivador e mantenha um tom de tutor profissional.`;
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-emerald-950/60 via-zinc-900 to-teal-950/40 border border-emerald-500/30 p-6 shadow-2xl">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10">
          ...
          <div className="flex items-center gap-2 self-start md:self-auto">
            {/* Botão de Sugestões de Perguntas */}
            <button
              ...
              className={cn(
                ...
                showSuggestions
                  ...
                  : "bg-zinc-800/80 hover:bg-zinc-800 text-zinc-400 hover:text-amber-300 border-zinc-700/50"
              )}
              title="Alternar Perguntas Sugeridas"
            >
      ...
      {/* Caixa do Chat */}
      <div className="bg-zinc-900 border border-zinc-800 rounded-3xl overflow-hidden shadow-2xl flex flex-col h-[600px] md:h-[680px] min-h-[500px]">
        {/* Lista de Mensagens */}
        ...
        {/* Sugestões Rápidas: aparecem no início ou quando solicitadas, e saem automaticamente após a pergunta e respostas relevantes */}
        {showSuggestions && !isTyping && (
          <div className="px-4 py-3 border-t border-zinc-800/80 bg-zinc-950/80 shrink-0 animate-in fade-in duration-200">
            <div className="flex items-center justify-between mb-2">
              <div className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider flex items-center gap-1.5">
                <Lightbulb className="w-3.5 h-3.5 text-amber-400" />
                <span>Perguntas Sugeridas (saem após o envio da dúvida):</span>
              </div>
              <button
                onClick={() => setShowSuggestions(false)}
                className="text-zinc-500 hover:text-zinc-300 p-1 rounded-lg hover:bg-zinc-800 transition-colors"
                title="Fechar sugestões e liberar caixa do chat"
              >
            ...
            <div className="flex items-center gap-2.5">
              ...
              {!showSuggestions && (
                <button
                  ...
                  className="text-amber-400/80 hover:text-amber-300 underline flex items-center gap-1 font-medium transition-colors cursor-pointer"
                >
